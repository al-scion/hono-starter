...
import { Asana, Github, Linear, Neon, Notion, PayPal } from '@/lib/icons';
...
export const useMcpStore = create<McpStore>((set, get) => ({
  ...
  removeMcpServer: async (id: string) => {
    await api.mcp.removeMcp.$delete({ json: { id } });
  },
  addMcpServer: async (name, url) => {
    // Check if there's already an authenticating server with this name
    const existingServer = Object.entries(get().mcpState.servers).find(
      ([_, server]) =>
        server.name === name &&
        server.auth_url &&
  ...
  enabledMcp: new Set(),
  addEnabledMcp: (mcp) =>
    set({ enabledMcp: new Set([...get().enabledMcp, mcp]) }),
  removeEnabledMcp: (mcp) =>
    set({
      enabledMcp: new Set([...get().enabledMcp].filter((m) => m !== mcp)),
    }),
}));

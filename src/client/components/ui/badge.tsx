...
type IconComponentType = React.FunctionComponent<{ className?: string; strokeWidth?: string | number }>;
...
export const filledColors: Record<BadgeColors, { root: string; addon: string; addonButton: string }> = {
    ...
    pink: {
        root: "bg-pink-50 text-utility-pink-700 ring-utility-pink-200",
...
export const BadgeWithDot = <T extends BadgeTypes>(props: BadgeWithDotProps<T>) => {
    ...
    return (
        <span className={cn(colors.common, sizes[type][size], colors.styles[color].root, className)}>
            <svg className={cn(colors.styles[color].addon, "size-1.5 rounded-full")} width="8" height="8" viewBox="0 0 8 8" fill="none" aria-hidden="true">
